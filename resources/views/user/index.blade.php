...
@section('content')
    <h5 class="card-title">{{ __('messages.User List') }}</h5>
    <a href="{{route('user.create')}}" class="btn btn-success"><i class="bi bi-star me-1"></i>{{ __('messages.User Party') }}</a>
    {{--    <button type="button" class="btn btn-primary"  href="{{ route('parties.create') }}"><i class="bi bi-star me-1"></i>{{ __('Create Party') }}</button>--}}
        ...
        @if(!empty($users) && $users->count())
            @foreach($users as $key => $value)
                ...
                <tr>
                    ...
                    <td>
                        {{--                        @can('edit')--}}
                        <a href="{{route('user.edit',$value->id)}}" class="btn btn-sm btn-primary">{{ __('messages.Edit')}}</a>
