...
@section('content')
    <h5 class="card-title">{{ __('messages.Sub Account List') }}</h5>
    <a href="{{route('sub_account.create')}}" class="btn btn-success"><i class="bi bi-star me-1"></i>{{ __('messages.Create Sub Account') }}</a>
    {{--    <button type="button" class="btn btn-primary"  href="{{ route('parties.create') }}"><i class="bi bi-star me-1"></i>{{ __('Create Sub Account') }}</button>--}}
    ...
    <table class="table datatable">
        ...
        <tr>
            {{--            <th scope="col">#</th>--}}
            <th scope="col">ID</th>
            <th scope="col">{{ __('messages.Sub Account Name') }}</th>
        ...
        <tbody>
        @if(!empty($subAccounts) && $subAccounts->count())
            @foreach($subAccounts as $key => $value)
                ...
                <tr>
                    ...
                    <td>
                        {{--                        @can('edit')--}}
                        <a href="{{route('sub_account.edit',$value->id)}}" class="btn btn-sm btn-primary">{{ __('messages.Edit')}}</a>
