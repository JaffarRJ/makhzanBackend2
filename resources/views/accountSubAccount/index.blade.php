...
@section('content')
    ...
    <a href="{{route('account_sub_account.create')}}" class="btn btn-success"><i class="bi bi-star me-1"></i>{{ __('messages.Manage Account') }}</a>
    <a href="{{route('account.create')}}" target="_blank" class="btn btn-success"><i class="bi bi-star me-1"></i>{{ __('messages.Add Account') }}</a>
    <a href="{{route('sub_account.create')}}" target="_blank" class="btn btn-success"><i class="bi bi-star me-1"></i>{{ __('messages.Add Sub Account') }}</a>
    ...
    <table class="table datatable">
        ...
        <tr>
            ...
            <th scope="col">{{ __('Sub Accounts') }}</th>
            <th scope="col">{{ __('Status') }}</th>
            <th scope="col">{{ __('Actions') }}</th>
        </tr>
        </thead>
        <tbody>
        @if(!empty($accountSubAccounts) && $accountSubAccounts->count())
            @foreach($accountSubAccounts as $key => $value)

                <tr>
                    {{--                    <td></td>--}}
                    <td>{{$value->id}}</td>
                    <td>{{$value->account->name}}</td>
                    <td>{{$value->subAccount->name}}</td>
                    <td>
                        @if($value->is_active)
