...
use App\Http\Requests\PartyTransaction\DeleteRequest;
use App\Http\Requests\PartyTransaction\DetailRequest;
use App\Http\Requests\PartyTransaction\ListingRequest;
use App\Http\Requests\PartyTransaction\StoreRequest;
use App\Http\Requests\PartyTransaction\UpdateIsActiveRequest;
use App\Http\Requests\PartyTransaction\UpdateIsShowRequest;
use App\Http\Requests\PartyTransaction\UpdateRequest;
...
class PartyTransactionController extends Controller
{
    ...
    public function listing(ListingRequest $request)
    {
        ...
        $partyTransactions = $query->paginate($this->pagination);
        return view('partyTransaction.index', compact('partyTransactions'));
//        return successWithData(GENERAL_FETCHED_MESSAGE, $data);
    ...
    public function create()
    {
        ...
        return view('partyTransaction.create',compact('parties','transactions'));
    }
...
//                    return error(GENERAL_ERROR_MESSAGE, ERROR_400);
            ...
            return redirect()->route('party_transaction.listing')
                ->with('success', __('Manage Party created successfully.'));
...
//                return error(GENERAL_ERROR_MESSAGE, ERROR_400);
            ...
            DB::commit();
            return redirect()->route('party_transaction.listing')
                ->with('success', __('Manage Party updated successfully.'));
//            return successWithData(GENERAL_SUCCESS_MESSAGE, $model->fresh());
        } catch (QueryException $e) {
            DB::rollBack();
            return error($e->getMessage(), ERROR_500);
    ...
    public function getTransactions(Request $request)
    {
        ...
        $transactions = Transaction::all();
        $partyTransaction = PartyTransaction::where('party_id', $inputs['party_id'])->get();
        // You can return the transactions as HTML, JSON, or in any format you prefer
        return view('partyTransaction.render_file', ['transactions' => $transactions,'partyTransaction' => $partyTransaction]);
    }
}
